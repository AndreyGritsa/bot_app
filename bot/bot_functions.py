...
# function to fast login to the steam account and launch the game
def login():
    while True:
        login = input("Login: ")
        password = input("Password: ")
        with open('bash.bat', 'r+') as f:
            f.truncate()
            f.write((
                        "taskkill /im Steam.exe /T /F\n"
                        "taskkill /im NewWorld.exe /T /F\n"
                        "start /low C:\PROGRA~2\Steam\Steam.exe "
                        "-login {0} {1} -applaunch 1063730"). \
                    format(login, password))
            f.close()

        os.startfile(r'C:\Python\Bot\bash.bat')
...
def full_rotation(server, names_asmo, names_asmo_plus_runic, names_asmo_plus_runic_plus_phoenix,
                  names_asmo_plus_phoenix):
    ...
    while 1:
        z, num = check_name(names_asmo, names_asmo_plus_runic, names_asmo_plus_runic_plus_phoenix,
                            names_asmo_plus_phoenix)
        a = a + list(z)
