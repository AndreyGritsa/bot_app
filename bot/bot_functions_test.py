...
#write today's date to the history of transactions
def date():
...
def check_name(names1, names2, names3, names4):
    ...
    im_for_check = pytesseract.image_to_string(cv2.cvtColor(nm.array(im), cv2.COLOR_BGR2GRAY))
...
def full_rotation(names_asmo, names_asmo_plus_runic, names_asmo_plus_runic_plus_phoenix, names_asmo_plus_phoenix):
    date()
    a = []
    z = []
    second_trade = []
    ...
    while 1:
        ...
        a = a + list(z)
        print("Second trade variable is: %r" % second_trade)
        print('a: %r' % a)
